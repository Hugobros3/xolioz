    ...
    init {
        scheduler.scheduleSyncRepeatingTask(plugin, {
            for (player in plugin.gameWorld.players) {
                ...
                val currentPlace = plugin.placesNames.getPlayerPlaceName(player)
                if (profile.lastPlace != currentPlace) {
                    profile.lastPlace = currentPlace
                    player.sendMessage(ChatColor.GRAY.toString() + "~" + currentPlace)
                }
